...
class Curl
{
    ...
    )
    {
        ...
        if ($proxy) {
            ...
            curl_setopt( $curl, CURLOPT_PROXYUSERPWD, $proxy['CURLOPT_PROXYUSERPWD'] ); // http代理认证账号，username:password的格式
     ...
     *
     * @param string $url 指定URL完整路径地址
     * @param string|null $cacert_url 指定当前工作目录绝对路径
     ...
     * @return bool|string
     */
    public static function post_header(string $url, string $cacert_url = '', string $para = '')
    {
        $header  = [];
     ...
     * @param string $url 指定URL完整路径地址
     * @param string|null $cacert_url 指定当前工作目录绝对路径
     * @param bool|null $getHeader
     * @param null $headers 头部信息
     * @param null $proxy 是否使用代理
     * @return array|bool|string
     */
    public static function get(string $url, string $cacert_url = null, bool $getHeader = null, $headers = null, $proxy = null)
    {
        $curl = curl_init( $url );
        curl_setopt( $curl, CURLOPT_HEADER, 0 ); // 过滤HTTP头
        curl_setopt( $curl, CURLOPT_RETURNTRANSFER, 1 ); // 显示输出结果
     ...
     * @return string
     */
    public static function buildUrlDataStr(string $url, array $data): string
    {
        $paramStr = '';
    ...
    protected function next(): bool
    {
        ...
        curl_setopt( $conn, CURLOPT_SSL_VERIFYPEER, (bool)$this->certificate ); // SSL证书认证
